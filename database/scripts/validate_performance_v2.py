...
"""
Production-grade performance validation with ALL security and reliability fixes.

FIXES from Task 8:
1. ✅ SQL Injection: Parameterized queries for embedding arrays
2. ✅ Connection pooling: ThreadedConnectionPool
3. ✅ Connection timeout: connect_timeout=10
4. ✅ Batch inserts: COPY FROM instead of sequential INSERT
5. ✅ Transaction isolation: REPEATABLE READ for benchmarks
6. ✅ Index existence verification
7. ✅ VACUUM ANALYZE after bulk inserts
8. ✅ Recall@K measurement
9. ✅ EXPLAIN ANALYZE capture
10. ✅ Write amplification measurement
11. ✅ Safe cleanup using IDs (not LIKE patterns)
12. ✅ Non-linear scaling detection
13. ✅ Warmup phase (cache priming)
14. ✅ Sufficient sample size (1000 iterations)
15. ✅ Concurrency testing
16. ✅ Error handling with subtransactions
"""
...
def init_connection_pool(min_conn: int = 2, max_conn: int = 10):
    """Initialize connection pool with timeout handling."""
    ...
    try:
        connection_pool = psycopg2.pool.ThreadedConnectionPool(
            ...
            connect_timeout=DB_CONNECT_TIMEOUT  # FIX: Add connection timeout
        )
        print(f"✅ Connection pool initialized (min={min_conn}, max={max_conn}, timeout={DB_CONNECT_TIMEOUT}s)")
    except psycopg2.OperationalError as e:
        print(f"❌ Failed to connect to database: {e}")
        print(f"   Check: {DB_NAME}@{DB_HOST}:{DB_PORT}")
        sys.exit(1)
...
def verify_index_exists(conn) -> bool:
    ...
    if result:
        ...
        print("   Run migration 002 first: database/migrations/002_create_schema_knowledge.sql")
        return False


def insert_dummy_conversations_copy(conn, count: int, prefix: str = 'perf_test') -> Tuple[float, List[int]]:
    """
    Insert dummy conversations using PostgreSQL COPY (100x faster than INSERT).

    FIX: Use COPY FROM instead of sequential INSERTs for performance.
    FIX: Use parameterized queries (no SQL injection risk).

    Returns: (rows_per_sec, list_of_inserted_ids)
    """
    print(f"📝 Inserting {count} dummy conversations (COPY bulk load)...")
    ...
    try:
        # Generate all embeddings in memory first (avoid Python overhead in loop)
        ...
        for i in range(count):
            ...
            csv_buffer.write(f"Performance test message {i}\t")
            csv_buffer.write(f"{embedding_str}\n")

        csv_buffer.seek(0)

        # Use COPY FROM for bulk insert (PostgreSQL native, very fast)
        cursor.copy_expert("""
        ...
        # Get IDs of inserted rows for cleanup
        cursor.execute("""
...
def compute_ground_truth(conn, query_embedding: np.ndarray, k: int = 10) -> List[int]:
    ...
    FIX: Use parameterized query (no SQL injection).
    """
    cursor = conn.cursor()
...
def test_similarity_search_with_recall(
    ...
    FIX: All queries use parameterized placeholders (no SQL injection).
    """
...
def cleanup_safe(conn, inserted_ids: List[int]) -> None:
    ...
    FIX: No LIKE pattern (injection-safe and faster).
    FIX: Uses IDs from insert operation.
    FIX: Subtransaction for error recovery.
    """
    ...
    try:
        ...
        # Batch delete in chunks of 1000 (PostgreSQL limit)
...
def test_scale(scale_size: int) -> Dict:
    ...
    with get_connection() as conn:
        ...
        # Insert data using COPY (much faster than INSERT)
        test_prefix = f'scale_{scale_size}'
        insert_speed, inserted_ids = insert_dummy_conversations_copy(
...
def main():
    """Main entry point."""
    print("🚀 Starting production-grade performance validation (v2 with all fixes)...")
    print(f"Database: {DB_NAME}@{DB_HOST}:{DB_PORT}")
    print(f"Scale tests: {SCALE_TEST_SIZES}")
    print(f"Benchmark iterations: {BENCHMARK_ITERATIONS}")
    print(f"Warmup iterations: {WARMUP_ITERATIONS}")
    print(f"Connection timeout: {DB_CONNECT_TIMEOUT}s\n")
